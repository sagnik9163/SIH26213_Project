...
def friendly_time(date_str):
    try:
        # Parse the raw UTC time from the database
        dt_utc = datetime.strptime(date_str.replace("Z", ""), "%Y-%m-%dT%H:%M:%S")
...
def check_alerts():
    ...
    alerts = []
    for r in rows:
        date_str = r["received_at"]
        try:
            dt_utc = datetime.strptime(date_str.replace("Z", ""), "%Y-%m-%dT%H:%M:%S")
            # Add the exact same IST offset here
            dt_ist = dt_utc + timedelta(hours=5, minutes=30)
            friendly = dt_ist.strftime("%I:%M %p")
        except Exception:
            friendly = date_str
